...
public class SQLServerRealStatisticDaoImpl implements RealStatisticDao {

	/**
	 * 获取当天某时间段内的充值总额  
	 */
	public Map<String, Integer> queryRealSubscribe(String time) {
		String sql = "select DATEPART(hh, time) as h, productId, sum(amount) as sum from [SubscribeRecord] where (time >= '"+
						time +" 00:00:00') and (time <='" + time + " 23:59:59') group by DATEPART(hh, time),productId order by DATEPART(hh, time) desc";

...
function post(url,frm,btn){var data='reqTimes='+new Date().getTime();if(frm,url){data=data+"&"+frmval(frm);}var ajax=new Ajax(function(){eval(ajax.Data);$(frm).reset();btn.disabled=false;},function(){alert('鏁版嵁鎻愪氦澶辫触!');btn.disabled=false;});ajax.post(url,data);}
function ajaxdel(url,id){var ajax=new Ajax( function(){if(ajax.Data=='ok'){hide(id);}else{alert(ajax.Data);}});ajax.get(url,"reqTime="+new Date().getTime());} // 鍒犻櫎椤圭洰
...
function msgbox(obj,maxbox){
	...
	$(t).innerHTML='<span onclick="msgboxclose();" title="鍏抽棴"></span>'+tit;
	if(src){
		$(m).innerHTML=$(src).innerHTML;
	}else{
		if(frm){url=frmval(frm,url);}
		$(m).innerHTML='<div id="loading">'+wait+'</div>';
		var ajax=new Ajax(function(){$(m).innerHTML='<p>'+ajax.Data+'</p>';},function(){$(m).innerHTML='<p>鏈嶅姟鍣ㄦ棤鍝嶅簲,璇风◢鍚庡啀璇�/p>';});
...
default :if(name.length==0){name=val};fck.InsertHtml(' <a href="'+val+'">涓嬭浇: '+name+'</a> ');break;}}
// 鏃堕棿閫夋嫨鍣�
var allMonth=[31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
//var allNameOfWeekDays=["涓�,"浜�, "涓�, "鍥�, "浜�, "鍏�, "鏃�];

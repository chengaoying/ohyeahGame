...
<%
	...
	PageModel<PurchaseProp> pageModel2 = (PageModel<PurchaseProp>)request.getAttribute("pageModel2");
...
<table style="margin: 0pt auto; padding-top: 10px;" height="101" bgcolor="#cccccc" border="0" cellpadding="3" cellspacing="1" width="98%">
	         ...
	         <select name="productId" id="game">
	            <option value="请选择查询的游戏" selected="selected">选择查询的游戏</option>
	             <%
	               ...
	               for(Iterator<Product> it = productList2.iterator(); it.hasNext();){
  ...
  <tr bgcolor="#f5f5f5">
	...
	<td align="center" valign="middle" ><%=pageModel2.getTotalPrice()%>元</td>
...
<table style="margin: 0pt auto; padding-top: 10px;" height="101" bgcolor="#cccccc" border="0" cellpadding="3" cellspacing="1" width="98%">
  ...
  <tr bgcolor="#f5f5f5">
	...
	<td align="center" valign="middle" >购买总额(元)</td>
     ...
     List list = pageModel2.getList();
     if(pageModel2.getList().size() == 0){
    	 pageModel2.setPageNo(0);
     }
     //System.out.println("pagemodel:" + pageModel2.getList().size());
    // System.out.println("getTotalPages:" + pageModel2.getTotalPages());
 ...
 <%} %>  
  ...
  <tr>
    <td colspan="14" height="24" bgcolor="#F8FCF6"><div class="page"><span><strong><%=pageModel2.getPageNo() %>/<%=pageModel2.getTotalPages() %></strong></span><a href="servlet/PropPurchaseServlet?command=<%=command %>&pageNo=<%=pageModel2.getPreviousPageNo() %>&productId=<%=productId %>&startTime=<%=sTime %>&endTime=<%=eTime %>"><img src="image/pre.gif" height="16" width="21"/></a>&nbsp;<a href="servlet/PropPurchaseServlet?command=<%=command %>&pageNo=<%=pageModel2.getNextPageNo() %>&productId=<%=productId %>&startTime=<%=sTime %>&endTime=<%=eTime %>"><img src="image/next.gif" height="16" width="21"/></a></div></td>
	</tr>
</tbody></table>

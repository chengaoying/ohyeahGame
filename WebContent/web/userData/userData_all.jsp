...
<table style="margin: 0pt auto; padding-top: 10px;" height="101" bgcolor="#cccccc" border="0" cellpadding="3" cellspacing="1" width="98%">
  ...
  <tr bgcolor="#f5f5f5" >
	...
	%>
    <tr bgcolor="#f5f5f5">
    <td height="24" align="center" valign="middle" width="50%"><a id="id" href="servlet/UserRecordServlet?pageNo=1&subscribeId=subscribe&userId=<%=userDataAnalysis.getUserId() %>"><%=userDataAnalysis.getUserId() %></a></td>
	<td align="center" valign="middle" width="50%"><%=userDataAnalysis.getPrice() %></td>

  </tr>
  <%} %>
 <tr bgcolor="#f5f5f5">
    <td height="24" align="center" valign="middle" width="50%">合计</td>
	<td align="center" valign="middle" width="50%"><%=pageModel.getTotalPrice() %></td>
  ...
  <tr>
    ...
    <a href="servlet/UserDataServlet?command=<%=command %>&pageNo=<%=pageModel.getPreviousPageNo() %>&startTime=<%=sTime %>&endTime=<%=eTime %>">
    <img src="image/pre.gif" height="16" width="21"/></a>&nbsp;
    <a href="servlet/UserDataServlet?command=<%=command %>&pageNo=<%=pageModel.getNextPageNo() %>&startTime=<%=sTime %>&endTime=<%=eTime %>">

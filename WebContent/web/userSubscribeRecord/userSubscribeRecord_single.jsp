...
<table style="margin: 0pt auto; padding-top: 10px;"  bgcolor="#cccccc" border="0" cellpadding="3" cellspacing="1" width="98%">
 ...
 <%} %>
  <tr>
    ...
    <a href="servlet/SubscribeStatisticServlet?pageNo=<%=pageModel.getPreviousPageNo() %>&startTime=<%=startTime %>&endTime=<%=endTime %>&command=<%=command %>&productId=<%=productId %>"><img src="image/pre.gif" height="16" width="21"/></a>&nbsp;
    <a href="servlet/SubscribeStatisticServlet?pageNo=<%=pageModel.getNextPageNo() %>&startTime=<%=startTime %>&endTime=<%=endTime %>&command=<%=command %>&productId=<%=productId %>"><img src="image/next.gif" height="16" width="21"/></a></div></td>
  </tr>
</tbody></table>
